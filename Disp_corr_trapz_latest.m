...
function [Signal_corr,Time]=Disp_corr_trapz_latest()
clc;
close all;
%% input parameters
Fs=100e6;
...
% inverse fft
   ...
   for i=2:Np
       if i<=Np/2+1
           ...
           phi(i)=2*pi*freq(Np+2-i)*z*(1/co-1/cn(i,1));
       end
   end
   figure;
...
%% Dispersion Correction
   ...
   figure;
   plot(freq,abs(F1),'-ob','LineWidth',2);
   hold on;
   plot(freq,abs(F_new),'-or','LineWidth',2);
