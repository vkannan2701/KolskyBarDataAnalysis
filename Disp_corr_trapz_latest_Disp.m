...
function []=Disp_corr_trapz_latest_Disp(Signal)
clc;
close all;
%% Input parameters
Fs=100e6;
...
%% Create and plot the artificial trapezoid
% Signal=zeros(Np,1);
% Signal(1,1)=0;
% for i=2:Np
%     if Time(i)<=20e-6
%         Signal(i)=0;
%     else if Time(i)<=23e-6
%             Signal(i)=(0.002/3e-6)*(Time(i)-20e-6)+0;
%         else if Time(i)<=57e-6
%                 Signal(i)=0.002;
%             else if Time(i)<=60e-6
%                     Signal(i)=(-0.002/3e-6)*(Time(i)-57e-6)+0.002;
%                 else
%                     Signal(i)=0;
%                 end
%             end
...
%% Dispersion Relation for a cylindrical bar
   ...
   title('Bancroft Data (\nu=0.3) : C_n/C_o Vs d/\lambda','FontSize',18,'FontName','Arial Narrow');
...
% inverse fft
   ...
   for i=2:Np
       if i<=(Np-1)/2+2
           ...
           phi(i)=2*pi*freq(Np+1-i)*z*(1/co-1/cn(i,1));
       end
   end
   figure;
...
%% Dispersion Correction
   ...
   figure;
   plot(freq,(F1),'-ob','LineWidth',2);
   hold on;
   plot(freq,(F_new),'-or','LineWidth',2);
   ...
   plot(Time(1:Np),Signal_reconst(1:Np),'-b','LineWidth',2);
   hold on;
   plot(Time(1:Np),Signal_corr(1:Np),'-r','LineWidth',2);
   hold off;
end
